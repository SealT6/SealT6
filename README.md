# 👋 Hi, I'm Sean

🎓 IT Graduate | 🛡️ Cybersecurity Student | 💻 Aspiring Security Professional

I hold an **Associate’s Degree in Information Technology**, and I am currently pursuing a **Bachelor’s Degree in Cybersecurity**.  
I’m passionate about securing systems, learning how attacks work, and building practical skills through hands-on projects.

---

## 🎓 Education

**Bachelor of Science in Cybersecurity** *(In Progress)*  
📍 Full Sail University  
📅 Expected Graduation: Summer 2026

**Associate of Science in Information Technology**  
📍 Full Sail University  
📅 Graduated: June 2025

---

## 🛠️ Technical Skills

### Cybersecurity
- Network Security Fundamentals
- Threat Analysis & Risk Management
- Vulnerability Assessment
- Security Policies & Best Practices
- Basic Incident Response

### IT & Systems
- Windows & Linux Administration
- Networking (TCP/IP, DNS, DHCP)
- Active Directory (Basics)
- Hardware & Software Troubleshooting

### Tools & Technologies
- Wireshark
- Nmap
- Kali Linux
- VirtualBox / VMware
- Git & GitHub

### Programming / Scripting
- Python (Intermediate)
- Bash (Basics)
- PowerShell (Intro)

---

## 📂 Projects

### 🔐 Cybersecurity Projects
- **Network Vulnerability Scan**
  - Used Nmap and Wireshark to identify open ports and potential risks
  - Documented findings and mitigation strategies

- **Home Lab (Virtual Environment)**
  - Built a virtual lab using VirtualBox
  - Configured Windows and Linux machines for security testing

### 💻 IT Projects
- **PC Build & Troubleshooting Guide**
  - Assembled and configured desktop systems
  - Diagnosed and resolved hardware/software issues

- **Basic Network Setup**
  - Configured routers, switches, and IP addressing
  - Implemented basic security settings

*(More projects coming as I progress through my degree)*

---

## 📜 Certifications (or In Progress)

- CompTIA A+ *(In Progress)*
- CISSP *(Planned)*
- CompTIA Security+ *(Planned)*

---

## 🎯 Career Interests

- Cybersecurity Analyst
- SOC Analyst (Tier 1)
- IT Support / Help Desk
- Network Security
- Blue Team / Defensive Security
- Red Team / Offensive Security

---

## 📫 Contact

- **Email:** seanredd2002@gmail.com
- **LinkedIn:** [[https://www.linkedin.com/in/sean-redding-aa503a293/]
- **GitHub:** You’re already here 🙂

---

## 🚀 Goals

- Gain real-world cybersecurity experience
- Complete hands-on labs and capture-the-flag challenges
- Earn industry-recognized certifications
- Secure an internship or entry-level cybersecurity role

---
